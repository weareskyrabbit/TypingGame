...
(function(window, document) {
  ...
  let wordJP1 = []; 
  let wordJP2 = [];
  let easyWord1 = ['桜', '海', '山', '空', '川', '花', '星', '月', '森', '風', '雨', '雪', '火', '水', '木', '土', '金', '銀', '鉄', '石']
  let easyWord2 = ['さくら', 'うみ', 'やま', 'そら', 'かわ', 'はな', 'ほし', 'つき', 'もり', 'かぜ', 'あめ', 'ゆき', 'ひ', 'みず', 'き', 'つち', 'きん', 'ぎん', 'てつ', 'いし']
  let normalWord1 = ['コンピュータ','インターネット','テレビジョン','エレベーター','エアコン','スマートフォン','タブレット','プロジェクト','カメラ','ゲーム']
  let normalWord2 = ['こんぴゅーた','いんたーねっと','てれびじょん','えれべーたー','えあこん','すまーとふぉん','たぶれっと','ぷろじぇくと','かめら','げーむ']
  let hardWord1 = ['一石二鳥','猿も木から落ちる','石の上にも三年','二兎を追う者は一兎をも得ず','花より団子','井の中の蛙大海を知らず','出る杭は打たれる','七転び八起き','猫に小判','馬の耳に念仏']
  let hardWord2 = ['いっせきにちょう','さるもきからおちる','いしのうえにもさんねん','にとをおうものはいっとをもえず','はなよりだんご','いのなかのかわずたいかいをしらず','でるくいはうたれる','ななころびやおき','ねこにこばん']
  let wordRs; // ローマ字データ1
  let wordR; // ローマ字データ2
  ...
  function word_normal(){
    ...
    image.src = '../static/images/nomal_image.png'; // easyボタンに対応する画像
    difficultyImage.style.display = 'block'; // 画像を表示
    message.classList.remove('hidden');
